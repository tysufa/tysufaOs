...
// Add reflow: true so the layout box shrinks alongside the visual key
#let smaller = keyle.themes.standard.with(
  text-args: (size: 4pt),
)
#let base-kbd = keyle.config(theme: smaller)
...
#let kbd(..args) = box(baseline: -15%, base-kbd(..args))

= Keybinds
#concept-block[
  #inline("Shared")

  #kbd("Alt+S") : toggle floating pane \
  #kbd("Ctrl+Alt+C") : Copy Zellij Selection \
  #kbd("Alt+N") : New pane \
  #kbd("Alt+S") : New floating pane \
  #kbd("Alt+T") : New stacked pane \
  #kbd("Alt+X") : Close focused pane \
  #kbd("Alt+1..5") : Select tab number\

  #inline[Normal Mode]

  #kbd("Alt+I") : Move tab to the left \
  #kbd("Alt+O") : Move tab to the right \
  #kbd("Alt+H") : Switch to mode move \


  #inline[Session Mode : #kbd("Ctrl+Y")]

  #kbd("Y") : Open session manager \
  #kbd("P") : Open plugin manager\
  #kbd("A") : Open the about page (contains news about last version) \
  #kbd("[ or ]") : Select the current nested session or ascend to the previous one \
  #kbd("F") : Focus on the nested session of the current pane \

  #inline[Scroll Mode : #kbd("Ctrl", "S")]
  
  #kbd("[") : Scroll to previous prompt \
  #kbd("]") : Scroll to next prompt \
  #kbd("M") : Select Command At Scroll Position \
  #kbd("C") : Copy the output of the last command \
  #kbd("S") : Search something in scroll mode (case sensitive) \
  #kbd("D/U") : Scroll half page down/up \
  #kbd("Ctrl+C") : Scroll to bottom \

  #inline[Pane Mode : #kbd("Ctrl", "S")]

  #kbd("C") : Rename pane\
  #kbd("D") : New pane down\
  #kbd("R") : New pane right\
  #kbd("E") : Toggle between embeded and floating\
  #kbd("F") : Toggle fullscreen on selected pane\
  #kbd("Z") : Toggle pane frames\

  #inline[Pane Mode : #kbd("Ctrl", "T")]

  #kbd("[ or ]") : Take the current pane to the left/right tab \
  #kbd("B") : Take the current pane and create a new tab with it \
  #kbd("N") : New tab\
  #kbd("R") : Rename tab\
  #kbd("S") : Synchronize the input of panes in the tab\
  #kbd("X") : Close the current tab\
